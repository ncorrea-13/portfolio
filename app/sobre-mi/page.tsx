    ...
    <>
      ...
      <Reveal>
        <div className="flex flex-col-reverse items-center gap-6 sm:flex-row sm:items-start">
          <div>
            <p>
              Soy un estudiante avanzado de Ingeniería en Sistemas de
              Información en la Universidad Tecnológica Nacional, Facultad
              Regional Mendoza. Actualmente tengo 37/41 materias aprobadas. Soy
              desarrollador full-stack en AYSAM. Me apasiona el desarrollo de
              software, y mi experiencia en Linux avanzado despertó interés en
              la infraestructura, la ciberseguridad y explotación de
              vulnerabilidades.
            </p>
            <p className="mt-3">
              Además, mantengo mi homeserver, donde administro infraestructura,
              servicios, contenedores, redes y privacidad aplicada a un uso
              diario real.
            </p>
          </div>
          <Image
        ...
        <ul className="space-y-2">
          {experience.map((item) => (
            <li key={item.role + item.period}>
              <strong className="font-sans text-heading">{item.role}</strong> (
      ...
      <Reveal>
        ...
        <ul className="space-y-2">
          {education.map((item) => (
            <li key={item.title}>
              <strong className="font-sans text-heading">{item.title}</strong> (
              {item.org}), {item.period}
            </li>
          ))}
        </ul>
      ...
      <Reveal>
        ...
        <p>
          Trabajo en equipo, comunicación efectiva, responsabilidad y
          adaptabilidad; sobre todo al pasar entre roles de desarrollo,
          infraestructura y de seguridad ofensiva, donde el contexto y las
          prioridades cambian rápido.
        </p>
      </Reveal>
      ...
      <Reveal>
        ...
        <p>
          Le doy tanta importancia a cómo está construido el código como a que
          funcione. Mi principal enfoque a la hora de construir software es
          mantener la filosofía KISS. Mantener el software lo más chico posible
          así se puede desacoplar de forma sencilla y reutilizar en diferentes
          proyectos; a menos que la situación amerite lo contrario.
        </p>
      </Reveal>
